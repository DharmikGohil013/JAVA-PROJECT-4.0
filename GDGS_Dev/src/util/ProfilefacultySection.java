...
public class ProfilefacultySection {
    ...
    public ProfilefacultySection(String email) {
        ...
        JLabel departmentLabel = new JLabel("Department:");  // New label for Department

        JTextField nameField = new JTextField();
        JTextField emailField = new JTextField();
        JTextField phoneField = new JTextField();
        JPasswordField passwordField = new JPasswordField();
        JTextField roleField = new JTextField();  // New text field for Role
        JTextField departmentField = new JTextField();  // New text field for Department
        ...
        roleField.setEditable(false);  // Make Role field uneditable
        departmentField.setEditable(false);  // Make Department field uneditable
        ...
        roleLabel.setBounds(xStart, yStart + ySpacing * 4, labelWidth, 30);  // Position Role label
        roleField.setBounds(xStart + labelWidth, yStart + ySpacing * 4, fieldWidth, 30);  // Position Role field
        departmentLabel.setBounds(xStart, yStart + ySpacing * 5, labelWidth, 30);  // Position Department label
        departmentField.setBounds(xStart + labelWidth, yStart + ySpacing * 5, fieldWidth, 30);  // Position Department field
        ...
        panel.add(roleLabel);
        panel.add(roleField);  // Add Role field
        panel.add(departmentLabel);
        panel.add(departmentField);  // Add Department field
        panel.add(backButton);

        // Fetch and display admin profile information
        fetchProfileInfo(nameField, emailField, phoneField, passwordField, roleField, departmentField);

        // Button action listeners
        backButton.addActionListener(e -> {
            frame.dispose();  // Close the current ProfileSection window
            new infacultysection(email);  // Return to the admin section
        });
    ...
    private void fetchProfileInfo(JTextField nameField, JTextField emailField, JTextField phoneField,
                                   JPasswordField passwordField, JTextField roleField, JTextField departmentField) {
        ...
        String query = "SELECT name, email, mo_number, password, rol, dept FROM faculty WHERE email = ?";
        ...
        try (Connection conn = DriverManager.getConnection(url, user, password);
            ...
            if (rs.next()) {
                ...
                roleField.setText(rs.getString("rol"));  // Set Role field
                departmentField.setText(rs.getString("dept"));  // Set Department field
            } else {
    ...
    public static void main(String[] args) {
        new ProfilefacultySection("faculty001@xyz.edu.in");  // Assume the user is already logged in as an admin
    }
}
